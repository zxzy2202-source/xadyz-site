...
export function LeadsListPage() {
  ...
  return (
    <div>
      ...
      {/* Inquiry Tabs — H-track v2.1 Standard 3.4 */}
      <div className="flex space-x-1 bg-gray-100 p-1 rounded-xl mb-6 w-fit">
        {[
          { id: 'all', label: '全部留言' },
          { id: 'contact', label: '咨询 (Contact)' },
          { id: 'inquiry', label: '询价 (Quote)' },
          { id: 'tender', label: '投标 (Tender)' },
          { id: 'sample', label: '样品 (Sample)' },
        ].map((tab) => (
          <button
            key={tab.id}
      ...
      <div className="bg-white rounded-xl shadow-sm p-5 mb-6 border border-gray-100">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          {/* Type Filter (Removed as select, replaced by Tabs above, keeping it as hidden to avoid breaking logic if needed, or just remove it) */}
          {/* We keep it as a backup or for distributor/oem which are not in tabs */}
          <select
            value={filterType}
            onChange={(e) => setFilterType(e.target.value)}
            className="px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-sm"
          >
            <option value="all">更多类型</option>
