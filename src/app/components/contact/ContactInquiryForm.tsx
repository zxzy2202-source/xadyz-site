...
export function ContactInquiryForm({ lang }: ContactInquiryFormProps) {
  ...
  const quickContactItems = {
    zh: [
      { icon: <MessageCircle size={20} className="text-green-500" />, label: 'WhatsApp', sub: '24h 内快速回复', href: COMPANY.contact.whatsappUrl },
      { icon: <MessageCircle size={20} className="text-sky-500" />, label: 'Telegram', sub: '长期沟通首选', href: COMPANY.contact.telegramUrl },
      { icon: <Mail size={20} className="text-gray-500" />, label: COMPANY.contact.email, sub: '正式询盘', href: `mailto:${COMPANY.contact.email}` },
    ],
    en: [
      { icon: <MessageCircle size={20} className="text-green-500" />, label: 'WhatsApp', sub: 'Quick reply within 24h', href: COMPANY.contact.whatsappUrl },
      { icon: <MessageCircle size={20} className="text-sky-500" />, label: 'Telegram', sub: 'For long-term communication', href: COMPANY.contact.telegramUrl },
      { icon: <Mail size={20} className="text-gray-500" />, label: COMPANY.contact.email, sub: 'Formal inquiries', href: `mailto:${COMPANY.contact.email}` },
    ],
    ru: [
      { icon: <MessageCircle size={20} className="text-green-500" />, label: 'WhatsApp', sub: 'Ответ в течение 24ч', href: COMPANY.contact.whatsappUrl },
      { icon: <MessageCircle size={20} className="text-sky-500" />, label: 'Telegram', sub: 'Для долгосрочного общения', href: COMPANY.contact.telegramUrl },
      { icon: <Mail size={20} className="text-gray-500" />, label: COMPANY.contact.email, sub: 'Официальные запросы', href: `mailto:${COMPANY.contact.email}` },
    ],
  };
