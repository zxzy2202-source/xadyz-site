...
import { ArrowRight, CheckCircle2, Factory, Clock, Shield, Globe, FileText, Box, Briefcase, Building2, Package, Printer, Calculator, Users } from 'lucide-react';
import { ProductPageCta } from '@/app/components/landing';
import { getContactInquiryUrl } from '@/app/lib/leadConfig';
import { content } from './NCRFormsPage.content';

export const pageAssets: PageAssetsConfig = {
  seoImage: { src: PLACEHOLDERS.hero.factoryLine, alt: "NCR forms production line" },
  hero: {
    src: PLACEHOLDERS.hero.factoryLine,
    alt: "NCR carbonless forms production and converting",
    overlay: "dark",
    focal: "left",
  },
  gallery: [
    { src: PLACEHOLDERS.product.ncrForms, alt: "NCR carbonless forms for documentation" },
    { src: PLACEHOLDERS.proof.qualityInspection, alt: "Quality inspection for NCR forms" },
    { src: PLACEHOLDERS.gallery.g2, alt: "NCR forms packaging and storage" },
  ],
  cards: {},
  proofs: [],
};
...
const getIcon = (iconName: string) => {
  const icons: Record<string, React.ReactNode> = {
    ...
    file: <FileText size={24} className="text-blue-600" />,
    calculator: <Calculator size={24} className="text-blue-600" />,
    briefcase: <Briefcase size={24} className="text-blue-600" />,
    building: <Building2 size={24} className="text-blue-600" />,
    users: <Users size={24} className="text-blue-600" />
  };
  return icons[iconName] || <Package size={32} className="text-blue-600" />;
};

interface NCRFormsPageProps {
  lang?: 'en' | 'ru' | 'zh';
  type?: string;
}

export const NCRFormsPage: React.FC<NCRFormsPageProps> = ({ lang = 'en', type }) => {
  ...
  const productJsonLd = {
    ...
    category: 'NCR Forms / Carbonless Copy Paper',
    offers: {
      '@type': 'AggregateOffer',
      priceCurrency: 'USD',
      lowPrice: '0.05',
    ...
    <>
      ...
      <main className="pt-32 pb-20">
        ...
        <section className="max-w-7xl mx-auto px-4 mb-24">
          <p className="text-lg text-gray-700 mb-10 max-w-4xl">{t.overview.paragraph}</p>
        ...
        <section className="bg-gradient-to-br from-gray-50 to-white py-16 mb-24">
          <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-gray-900 mb-12 text-center">{t.productTypes.sectionTitle}</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {t.productTypes.types.map((type) => (
                <div key={type.id} className="bg-white border-2 border-gray-200 rounded-2xl p-8 hover:border-blue-400 hover:shadow-xl transition-all flex flex-col">
                  <h3 className="text-2xl font-bold text-gray-900 mb-3">{type.title}</h3>
                  <p className="text-gray-600 mb-6 text-base">{type.shortDesc}</p>
        ...
        <section className="max-w-7xl mx-auto px-4 mb-24">
          <h2 className="text-3xl font-bold text-gray-900 mb-6 text-center">{t.manufacturing.sectionTitle}</h2>
          <p className="text-lg text-gray-700 mb-12 max-w-4xl mx-auto text-center">{t.manufacturing.intro}</p>
        ...
        <section className="bg-gradient-to-br from-blue-50 to-white py-16 mb-24">
          <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-gray-900 mb-12 text-center">{t.applications.sectionTitle}</h2>
